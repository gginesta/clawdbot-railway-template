...
- Cron `delivery.to` must be explicit: `channel:<discord_channel_id>` (never raw ids).
- Ops crons must not deliver into Telegram/webchat.
- Cron prompts must include a hard line: **"Do NOT post to any channel; return plain text only."**
...
- Never deliver maintenance/system events into interactive sessions (Telegram/webchat). If you need announcements, use Discord-only delivery.
