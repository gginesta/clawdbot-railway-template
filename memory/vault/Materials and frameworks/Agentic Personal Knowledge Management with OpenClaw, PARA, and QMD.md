...
The core of the system is a directory tree organized using Tiago Forte's
...
1. [summary.md](https://summary.md/)
    
    — A concise overview. This is what the agent loads first for quick context.
    
2. items.json — An array of atomic facts. Only loaded when the agent needs granular detail.
...
[2026-01-28.md](https://2026-01-28.md/)

├──

[2026-01-29.md](https://2026-01-29.md/)

├──

[2026-01-30.md](https://2026-01-30.md/)

└──

[2026-01-31.md](https://2026-01-31.md/)
...
The system implements this through recency weighting on the
...
- Hot (accessed in last 7 days) — Prominently included in
    
    [summary.md](https://summary.md/)
    
    . This is what the agent reaches for first.
    
- Warm (accessed 8–30 days ago) — Still included in
    
    [summary.md](https://summary.md/)
    
    , but at lower priority. Available but not front-of-mind.
    
- Cold (not accessed in 30+ days) — Omitted from
    
    [summary.md](https://summary.md/)
    
    entirely. But critically, cold facts are not deleted — they remain in items.json and can be retrieved via search at any time. Accessing a cold fact "reheats" it, bumping it back to Hot on next access.
...
3. Writes those facts to the appropriate entity in the knowledge graph
    
4. Updates daily notes with timeline entries
    
5. Bumps access metadata on any facts that were referenced
...
files are rewritten from active facts. This is where memory decay is applied:

1. Load all active facts from items.json
    
2. Sort by recency tier (Hot → Warm → Cold)
    
3. Within each tier, sort by accessCount (descending)
    
4. Write Hot and Warm facts into
    
    [summary.md](https://summary.md/)
    
5. Drop Cold facts from the summary (they remain in items.json)
...
always reflects your current mental model of an entity, not its complete history. The complete history lives in items.json for when you need it.
...
is a local indexing and retrieval tool that sits on top of the plain-file knowledge base. It indexes markdown files into a SQLite database and provides three search modes:
...
QMD returns ranked snippets with file paths and line numbers. The agent loads only what's relevant — a few paragraphs instead of the entire knowledge base. This is what makes tiered retrieval practical at scale:

[summary.md](https://summary.md/)

provides quick context, and QMD provides precision lookup when the summary isn't enough.
...
1. Start with the directory structure. Create the PARA folders and an
    
    [index.md](https://index.md/)
    
    . Don't overthink it.
    
2. Pick one active project and one important person. Create their
    
    [summary.md](https://summary.md/)
    
    and items.json. Get the schema right on two entities before scaling.
...
The source of this system draws heavily on Tiago Forte's

[Building a Second Brain](https://www.buildingasecondbrain.com/)

and the PARA method. The memory decay and atomic fact layers are original extensions designed specifically for AI agent memory.