...
103. **Calendar booking rules — Guillermo directive 2026-03-03.** (1) Always check conflicts first across ALL calendars. (2) Personal events → Shenanigans (family calendar) — ID unknown, NO ACCESS YET, must get before booking. (3) All bookings → also add Brinc "Busy [private]" block so colleagues see unavailability without details. (4) Brinc events → Brinc calendar visible (not private), no extra block needed. BLOCKER: no Shenanigans access = can't safely book personal slots. Ask Guillermo to share with service account.
