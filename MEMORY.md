...
82. **HTTP 200 health check ≠ version confirmed for webhook-updated agents.** Raphael nearly had config corrupted because fleet-update.py marked him as v2026.2.26 after webhook ACK, but he was still on v2026.2.25. Remote agents now marked `pending_update` (not `current`) until they self-report. Never send version-dependent scripts without confirmed version gate.
83. **PLAN-006 — Fleet Directive System (Feb 27).** /data/shared/pending-directives/<agent>/ queue. check_directives.py runner shared on volume. write_directive.py on Molty. REQUIRES_VERSION header gates execution. Molty 15-min cron live (bc60c335). Raphael + Leonardo need bootstrap cron (one-time Discord message from Guillermo).
