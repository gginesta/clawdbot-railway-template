...
// --- Dashboard password protection (upstream b50cff4) ---
...
function requireDashboardAuth(req, res, next) {
  ...
  if (req.path.startsWith("/hooks")) return next(); // allow OpenClaw webhook endpoints (upstream b178ea1)
...
// --- Gateway token injection (upstream b50cff4) ---
...
// Inject gateway token for WebSocket proxy requests (upstream 13ffd5d)
...
const server = app.listen(PORT, "0.0.0.0", async () => {
  ...
  // Sync gateway tokens in config with the current env var on every startup (upstream b50cff4).
...
server.on("upgrade", async (req, socket, head) => {
  // Note (upstream 13ffd5d): browsers cannot attach arbitrary HTTP headers (including
  // Authorization: Basic) in WebSocket handshakes. Do not enforce dashboard Basic auth
  // at the upgrade layer. The gateway authenticates at the protocol layer and we inject
  // the gateway token via proxy.on("proxyReqWs", ...) above.
  if (!isConfigured()) {
    socket.destroy();
    return;
  }
