...
Write final log to `/data/workspace/logs/overnight-tasks-${TODAY}.md`:
```
## ✅ Completed (N)
- Task name → Notion link (or "no output" if N/A)

## ⏭️ Skipped — already done/discussed (N)
- Task name: [memory evidence found]

## 🚩 Flagged — needs Guillermo input (N)
- Task name: [reason]
...
Post summary to #squad-updates (Discord channel 1468164181155909743):
```
🌙 Molty overnight — {DATE}
✅ Completed: N | ⏭️ Skipped: N | 🚩 Flagged: N | ❌ Failed: N
[one line per completed task with Notion link]
[one line per flagged item]
```

Update MC activity:
```
POST /api/activity — agentId: molty, type: task_update, title: "Overnight run complete — {DATE}", body: "[summary]", project: fleet
```
