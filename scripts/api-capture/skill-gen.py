...
def body_preview(body: Any, max_len: int = 2000) -> Optional[str]:
    if body is None:
        return None
...
# Load credentials env (unless this is a --dry-run preview)
if [[ -f "$ENV_FILE" ]]; then
  set -a
...
if [[ -z "${BASE_URL:-}" ]]; then
  if [[ $DRY_RUN -eq 1 ]]; then
    ...
    echo "Set BASE_URL (e.g., https://api.example.com) in $ENV_FILE" >&2
    exit 2
  fi
fi
...
if [[ "$URL" == *"{id}"* ]]; then
  if [[ -z "$ID" ]]; then
    if [[ $DRY_RUN -eq 1 ]]; then
      : # keep {id} placeholder for preview
    else
...
def main() -> int:
    ...
    ap.add_argument("--out-base", default="/data/shared/api-skills", help="Base output directory (default: /data/shared/api-skills)")
    args = ap.parse_args()
    ...
    for (method, norm_pq), count in endpoint_counts.most_common():
        ...
        endpoints.append({
            ...
            "sample_request_body": body_preview((sample.get("request") or {}).get("body")),
            "sample_response_body": body_preview((sample.get("response") or {}).get("body")),
...
"""
    ...
    # Write curl-commands.sh (prints one reproducible curl per generated endpoint)
    curl_sh = f"""#!/usr/bin/env bash
set -euo pipefail

DIR=\"$(cd \"$(dirname \"${{BASH_SOURCE[0]}}\")\" && pwd)\"

echo \"# curl commands for {domain}\"
echo \"# (each endpoint script prints an equivalent curl via --dry-run)\"
...
"""
    curl_path = os.path.join(out_dir, "curl-commands.sh")
    write_file(curl_path, curl_sh)
    chmod_x(curl_path)
...
- `curl-commands.sh` — prints a reproducible curl command set (`./curl-commands.sh`)
- `endpoints.json` — machine-readable endpoint catalog (method/path/auth detection + sanitized headers)
...
Print a full curl command set (one per endpoint):

```bash
./curl-commands.sh
```

Note: `--dry-run` works even if you haven't created the credentials env yet (it will use placeholder values).
...
If an endpoint fails (401/403/CSRF/required headers), re-run capture:

1. Start a fresh capture against the same domain.
2. In the browser, perform the exact action again (click/submit the form).
3. Re-generate the skill.

This MVP intentionally keeps scripts simple; some sites require session cookies or CSRF headers that only appear in certain flows.
